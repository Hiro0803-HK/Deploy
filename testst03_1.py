...
st.set_page_config(
    ...
    menu_items={
         ...
         #### ※諸注意※
         #### ーーーーーーーーーーーーーーーーーー
         ##### 当サイトは神戸電子専門学校の授業の一環として、私たち生徒7人で作成したものです。
         ##### 粗が目立つとは思いますが、  暖かい目で見て頂けたら幸いです。
         ##### 集めた画像は全て私たちが撮影したものです。
         #### ーーーーーーーーーーーーーーーーーー
         # MICLOSET
         ##### （メンズ専用）貴方にファッションに対して興味と知識を沸かせるwebアプリ
         名前の由来は「micro(小さい)」と「closet(クローゼット)」
         から来ています。
         このwebアプリは3ページ構成となっております。
         最終ページで貴方に似合う服のジャンルを表示します。
         しかし、主目的は貴方にファッションに興味を持って貰うことです。
         正直に入力する必要はありません。
